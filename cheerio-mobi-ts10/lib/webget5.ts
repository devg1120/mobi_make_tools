...
function gets_extract_sections() {
    ...
    let href_section_regex = new RegExp(
        config['extract_sections']['href_section_regex']
    )
    let href_article_regex = new RegExp(
        config['extract_sections']['href_article_regex']
    )
    ...
    try {
        client
            ...
            .then(function(result) {
                ...
                //var $a = result.$('.toc a');
                let $a = result.$(
                    `${config['extract_sections']['href_select_class']} a`
                )
...
//var _path_ = 'articles/' + filename
    ...
    client
		...
		let out = beautify.html(content2, beautifyOptions);
                fs.writeFileSync('content/' + recipe_slug + '/' + _path_, out)
