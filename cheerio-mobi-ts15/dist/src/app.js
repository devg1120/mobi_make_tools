    ...
    if (subcmd == "init") {
        if (configfile != null) {
            if (fs_1.default.existsSync(configfile)) {
                //file exists
                shelljs_1.default.cp('./config.yaml', '/tmp');
                shelljs_1.default.cp(configfile, './config.yaml');
            }
            else {
                console.log(chalk_1.default.red("config.yarm not foubd: " + configfile));
                return;
            }
        }
        else {
            console.log(chalk_1.default.red("init must be config filename  "));
            return;
        }
    }
