    ...
    */
    ...
    cheerio_httpcli_1.default
        ...
        if (basename.match(/root\.html$/)) {
            var head = result.$('h1');
            //var content = result.$(basename).nextUntil('h2')
            //var content = result.$(basename).next('p')
            var content = result.$('h1').parent().children('div').children('p').first();
            var all = head.toString() + content.toString();
            fs_1.default.writeFileSync('content/' + recipe_slug + '/' + _path_, all);
        }
        else if (basename.match(/^#/)) {
            //var content = result.$(basename)
            //var content = result.$(basename).siblings('h2')
            ...
            var content2 = result.$('body').html();
            fs_1.default.writeFileSync('content/' + recipe_slug + '/' + _path_, content2);
        }
    })
        .catch(function (err) {
        console.error(chalk_1.default.red('ERROR'), err);
