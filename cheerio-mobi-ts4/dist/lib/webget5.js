...
function gen_kindler_mobi() {
    ...
    /*
    console.log('----------');
    let S_ = JSON.stringify(SECTIONS, null, '\t');
    console.log(S_);
    console.log('----------');
    */
    ...
    console.log(dateformat_1.default(Date.now(), "dddd, mmmm dS, yyyy, h:MM:ss TT"));
    console.log(dateformat_1.default(Date.now(), "yyyy-mm-dd-hhMMss"));
    //return
    /// RUN kindlegen 
    /*
        ...
        let child = child_process.execFile("kindlegen",
                                           param,                     // args
                                        { cwd : output_dir },   // options
                                          (error, stdout, stderr) => {
    ...
    */
    let _infile_ = infile.substring(output_dir.length + 1);
    //let command = `kindlegen ${infile} -verbose -c2 -o ${outfile} `
